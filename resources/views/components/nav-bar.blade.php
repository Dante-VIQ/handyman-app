<header x-data="{ mobileMenuOpen: false }"
    class="relative flex flex-wrap items-center justify-between gap-4 mb-0 bg-white/70 backdrop-blur-sm py-3 px-4 sm:px-7 rounded-2xl sm:rounded-full border border-soft-blue/60 soft-shadow z-50">
    <!-- Logo -->
    <div class="flex items-center gap-4 z-20">
        <img src="/images/logo.png" alt="GB Handyman Solutions" class="w-16 h-16 sm:w-18 sm:h-18 object-cover">
        <a href="/"
            class="text-2xl sm:text-3xl font-bold bg-gradient-to-br from-[#166b8a] to-[#2e8cad] bg-clip-text text-transparent">
            GB
        </a>
    ...
    <!-- Desktop Navigation -->
    <nav class="hidden lg:flex flex-wrap items-center gap-1 text-sm font-medium mx-auto">
        <a href="/"
            class="px-5 py-2 rounded-full text-[#1e607b] hover:bg-soft-blue transition-all duration-200 {{ request()->is('/') ? 'bg-soft-green border border-soft-green font-semibold' : '' }}">
            Home
        </a>
        <a href="/about"
            class="px-5 py-2 rounded-full text-[#1e607b] hover:bg-soft-blue transition-all duration-200 {{ request()->is('about') ? 'bg-soft-green border border-soft-green font-semibold' : '' }}">
            About
        </a>
        <a href="/services"
            class="px-5 py-2 rounded-full text-[#1e607b] hover:bg-soft-blue transition-all duration-200 {{ request()->is('services') ? 'bg-soft-green border border-soft-green font-semibold' : '' }}">
            Services
        </a>
        <a href="/service-area"
            class="px-5 py-2 rounded-full text-[#1e607b] hover:bg-soft-blue transition-all duration-200 {{ request()->is('service-area') ? 'bg-soft-green border border-soft-green font-semibold' : '' }}">
            Service area
        </a>
        <a href="/gallery"
            class="px-5 py-2 rounded-full text-[#1e607b] hover:bg-soft-blue transition-all duration-200 {{ request()->is('gallery') ? 'bg-soft-green border border-soft-green font-semibold' : '' }}">
            Gallery
        </a>
        <a href="/contact"
            class="px-5 py-2 rounded-full text-[#1e607b] hover:bg-soft-blue transition-all duration-200 {{ request()->is('contact') ? 'bg-soft-green border border-soft-green font-semibold' : '' }}">
            Contact
        </a>

        @role('master|engineer')
            <a href="/analysis"
                class="px-5 py-2 rounded-full text-[#1e607b] hover:bg-soft-blue transition-all duration-200 {{ request()->is('analysis') ? 'bg-soft-green border border-soft-green font-semibold' : '' }}">
                Analysis
            </a>
        @endrole
        ...
        @auth
            <div class="relative group ml-2">
                <button
                    class="flex items-center gap-2 px-4 py-2 rounded-full bg-soft-blue border border-soft-blue font-semibold hover:bg-[#c5e0ff] transition">
                ...
                <div
                    class="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-lg border border-soft-blue/60 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
        ...
        @guest
            <div class="flex items-center gap-2 ml-2">
                <a href="{{ route('login') }}"
                    class="px-4 py-2 text-sm font-semibold text-gray-700 hover:text-[#166b8a] hover:bg-soft-blue rounded-full transition">
                    Login
                </a>
                <a href="{{ route('register') }}"
                    class="px-4 py-2 bg-[#166b8a] text-white rounded-full text-sm font-semibold hover:bg-[#0e4e63] transition shadow-sm">
                    Register
                </a>
            </div>
        @endguest
