<header x-data="{ mobileMenuOpen: false, scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 20 })"
    ...
    <div class="max-w-8xl mx-auto flex items-center justify-between pb-4">
        ...
        <a href="/" class="flex items-center gap-3 group">
            ...
            <div class="relative">
                <div
                    class="absolute inset-0 bg-amber-400/20 rounded-xl blur-md group-hover:bg-amber-400/30 transition-all duration-300">
                </div>
            ...
            @foreach ($navItems as $url => $item)
                <a href="{{ $url }}"
                    ...
                    <span class="relative z-10 flex items-center gap-2">
                        <i
                            class="{{ $item[1] }} text-sm {{ request()->is(ltrim($url, '/') ?: '/') ? 'text-amber-500' : 'text-gray-400 group-hover:text-amber-400' }}"></i>
                        {{ $item[0] }}
                    </span>
                    <!-- Animated underline -->
                    <span
                        class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-amber-400 group-hover:w-full transition-all duration-300 {{ request()->is(ltrim($url, '/') ?: '/') ? 'w-full bg-amber-500' : '' }}"></span>
                </a>
            @endforeach
                ...
                <div class="hidden lg:block">
                    <div class="relative" x-data="{ profileOpen: false }">
                        <button @click="profileOpen = !profileOpen"
                            ...
                            <i class="fas fa-chevron-down text-xs text-amber-600 transition-transform duration-200"
                        ...
                        <div x-show="profileOpen" @click.away="profileOpen = false"
                            ...
                            <div class="p-2">
                                <a href="/profile"
                                    class="flex items-center gap-3 px-4 py-2.5 text-gray-700 hover:bg-amber-50 rounded-lg transition">
                                    <i class="fas fa-user-cog text-amber-500 w-5"></i>
                                    Profile Settings
                                </a>
                                @role('master|engineer')
                                <a href="/analysis" class="flex items-center gap-3 px-4 py-2.5 text-gray-700 hover:bg-amber-50 rounded-lg transition">
            ...
            <nav class="p-3 space-y-1">
                @foreach ($navItems as $url => $item)
                    <a href="{{ $url }}" @click="mobileMenuOpen = false"
                        ...
                        <i
                            class="{{ $item[1] }} w-5 text-center {{ request()->is(ltrim($url, '/') ?: '/') ? 'text-amber-600' : 'text-gray-400' }}"></i>
                        {{ $item[0] }}
                    </a>
                @endforeach
